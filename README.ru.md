...
в Oracle Cloud: регистрация → API-ключ → автонастройка сети → охота за свободными
мощностями → скачивание SSH-ключа.

Лимиты Always Free: **2 OCPU / 12 ГБ** для бесплатных аккаунтов (Oracle втихую
урезал вдвое 15 июня 2026; PAYG-аккаунтам, по отзывам, по-прежнему бесплатно
доступно 4 OCPU / 24 ГБ).
...
- **Общий логин** (для команды): задайте `APP_LOGIN` и `APP_PASSWORD` в `.env`.
  Открывший сайт без токена видит страницу входа; после входа автоматически
  выдаётся персональная токен-ссылка (хэши токенов — в `data/tokens.json`).
- **Токены вручную**: дописать токен в `ACCESS_TOKENS` (`.env`) через запятую и
  `docker compose up -d` — активные охоты других пользователей возобновятся
  автоматически.

Удаление данных — кнопка «Удалить мои данные» в UI, либо
